...
io.on('connection', (socket) => {
  socket.emit('votingStatus', { votingOpen });

  socket.on('vote', (option) => {
    if (votingOpen && votes.hasOwnProperty(option)) {
      votes[option]++;
...
function handleAdminCommand(command) {
  switch (command.trim()) {
    case 'start':
      ...
      io.emit('votingStatus', { votingOpen });
      io.emit('reloadPage'); // Emit reload request to all clients
    ...
    case 'help':
      ...
      console.log('start - start voting\nstop - stop voting\nreset - reset votes\nstatus - check status\nexit - exit');
      break;
    default:
      console.log('Unknown command. Available commands: start, stop, reset, status, exit.');
  }
}
